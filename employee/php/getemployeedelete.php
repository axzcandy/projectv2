	...
	if(checkUserLoginValid()==true){
		if($_REQUEST['what']=="getallname"){
			...
			$user=$_SESSION['us'];
				...
				and employeeidpass.EID != '$user' AND employeeidpass.Statue!='locked'";
			...
			$eid=$_REQUEST['eid'];
			$sql="SELECT employee.EID, employee.EName, employee.DeptID, employee.Tel, employeeidpass.Statue FROM employee 
			 JOIN employeeidpass ON employee.EID = employeeidpass.EID WHERE employeeidpass.EID = '$eid' AND employeeidpass.Statue!='locked'";
			$result = mysqli_query($conn, $sql) or die("Error in Selecting " . mysqli_error($conn));	
			$emparray = array();
			while($row =mysqli_fetch_assoc($result))
			{
